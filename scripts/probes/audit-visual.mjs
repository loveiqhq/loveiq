...
for (const name of (process.env.DEVICES ?? "Pixel 7,iPhone 15 Pro,iPhone SE").split(",")) {
  ...
  // ── SURVEY ────────────────────────────────────────────────────────────
  /**
   * The network work is inside the try. An uncaught throw exits 1, which the
   * verifier reads as "the defect reproduced" — a defect nobody measured.
   * Verified before the fix: REPORT_ORIGIN=http://localhost:1 exited 1.
   */
  try {
    ...
    // No `problems` entry is pushed for this device, so the existing
    // "no device was measured" guard below turns this into exit 3.
    console.log(
