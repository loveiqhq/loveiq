...
│   │   ├── mcp/route.ts        # Company brain over MCP — Claude's recall layer (each person's own sign-in, or the shared token for unattended jobs)
│   │   ├── jarvis/             # Jarvis sign-in: sign-in (email a code), verify (code → session), decision (allow Claude), sign-out
...
│   ├── jarvis/connect/page.tsx # Where Claude sends a person to sign in to Jarvis (Supabase OAuth authorization path)
│   ├── .well-known/oauth-protected-resource/ # Tells Claude where to sign in to Jarvis (RFC 9728)
...
│   ├── brain/                  # server/ (ingest/ + search) + tests/ — company brain, reached by Claude over MCP (`/api/mcp`), not a chat product. Sources: repo docs, Gmail, Drive, Notion, Slack, Calendar, WhatsApp, GA4, Search Console, funnel rollups, Clarity, research evidence, shipped report copy, vocabulary, skills, people, decisions, notices (`list_sources` is the live list). Git commits stopped being indexed 2026-09-09; the Jira ingester is parked (decision 2026-08-28).
...
10. **Nurture Email Sequence:** `/api/cron/nurture-sequence` (hourly) reads `personal_report.created_date_time` and fans out to **one** stage — `72h_no_unlock` (`type Stage` in the route is the source of truth). It mints a per-user Stripe Promotion Code from `STRIPE_COUPON_50` (24h expiry, customer-restricted) → Resend send → idempotency write to `report_price_quote.metadata.nurtureEmailsSent[]` + `nurturePromoCodes[stage]`. Pricing 2.0 retired the earlier ladder — the 6h "report ready"/"unlock" reminders and the 30h/54h escalating discounts are gone; the 78h call invite went with the Calendly removal on 2026-09-14. Do not reintroduce them from an older copy of this file. Email CTA links carry `?promo=<code>&offer=1&pricingSessionId=…&utm_campaign=<stage>`. The `/report/[token]` page stashes `?promo=` in sessionStorage; checkout-session POST forwards it, server validates ownership via `resolveNurturePromo()`, pre-applies as `discounts:[]` on the Stripe session, and stamps `metadata.promoCode`/`metadata.promoStage` for attribution.

11. **Post-call 100% coupon (manual grant):** After the call, an admin clicks "Grant 100% coupon" on the submission detail → `/api/admin/submissions/[id]/grant-call-coupon` mints a one-time 100%-off code (`STRIPE_COUPON_100`, 14-day expiry), stores it under `report_price_quote.metadata.nurturePromoCodes.post_call`, emails the user a one-tap unlock link, and writes a `call_coupon_sent` `booking_event`. Redeemed via the normal `?promo=` checkout → $0 session → existing fulfillment unlocks `full_report`.
12. **Report session close:** The report page beacons `visibilitychange`→hidden and `pagehide` to `/api/report-session-end` (CSRF in the body, since `sendBeacon` cannot set headers), which stamps `ended_at` on the newest `report_session` for that submission. Deliberately NOT consent-gated, matching the row it closes — `report_session` is written server-side for every reader because `report_viewed` misses ~44% of opens. It is the only record of when a reader actually LEFT: `buildSubmissionJourney` feeds every session boundary into the measured "Report time" on the Slack journey message, and the two /admin session-duration cards read the same column.
...
| `GMAIL_MAILBOXES`                         | No           | Mailboxes the company-brain Gmail ingester reads, comma-separated. Unset means the credential's own mailbox. Reading a colleague's mail additionally requires Workspace domain-wide delegation — a user OAuth token only ever reaches its own mailbox, whatever scope it carries.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `GMAIL_EXCLUDE_MAILBOXES`                 | No           | Mailboxes to leave OUT of the company-brain Gmail walk, comma-separated, whatever the Workspace directory returns. Discovery already drops accounts that are SUSPENDED; this covers the case it does not — a colleague who has left while their account is still live, or mail the company simply does not want indexed. Matching ignores case and padding. Excluding a mailbox does NOT delete what is already indexed: the sweep keeps rows from any mailbox a run did not walk, so history is preserved and removing it stays a separate, deliberate act. Kept as configuration rather than code because this repository is public.                                                                                                                                                                                                                                                                                                                                                |
| `GMAIL_EXCLUDE_SUBJECTS`                  | No           | Subjects to keep OUT of the company-brain Gmail walk, comma-separated, applied as Gmail `-subject:` terms. Unlike the mailbox switch above this DOES remove what is already indexed: the filter runs on the listing, so an excluded thread never enters the seen-set and the normal sweep deletes its stored rows. Intended for a sibling project's issue-tracker mail — measured 2026-09-06, one such project's tickets took rank 1 on four of twelve ordinary LoveIQ questions and the entire top three on two of them, because generic engineering wording matches a ticket queue better than it matches a decision. Terms containing Gmail query operators are dropped rather than escaped, since Gmail reinterprets a malformed term instead of failing. Kept as configuration rather than code because this repository is public.                                                                                                                                               |
| `GOOGLE_WORKSPACE_ADMIN`                  | No           | Workspace admin to ask the directory as, when discovering every colleague's mailbox. An ordinary user is refused. Requires domain-wide delegation.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `GOOGLE_WORKSPACE_DOMAIN`                 | No           | Domain to enumerate (e.g. `loveiq.org`). With the admin above, the Gmail ingester discovers active mailboxes from the directory instead of a hand-kept list that goes stale when somebody joins or leaves.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
| `NEXT_PUBLIC_POSTHOG_HOST`                | For PostHog  | PostHog ingest host, e.g. `https://eu.i.posthog.com` (EU cloud, project 244778). **Must stay ABSOLUTE.** Since 2026-09-14 the BROWSER no longer uses it — it posts to the first-party path in `shared/analytics/posthog-proxy.ts`, which `next.config.js` rewrites to PostHog so ad blockers cannot match on hostname. This variable is still read by `features/analytics/server/posthog.ts` for the server-side purchase send, which runs in a Vercel function where a relative path has no origin to resolve against, and by `proxy.ts` to build the CSP. Pointing it at the proxy path breaks server-side purchase tracking silently — the send is best-effort and a bad URL logs nothing. A test asserts it.                                                                                                                                                                                                                                                                      |
...
| `SLACK_BRAIN_WEBHOOK_URL`                 | No           | The company brain's own alert channel: ingest failures, the nightly brief, corpus reachability, slow `brain-*` crons. Split out on 2026-09-14 because the brain posted more to the ops channel than everything else combined, which is how a channel stops being read. **Unset falls back to `SLACK_OPS_WEBHOOK_URL`**, so adding the variable is what moves the traffic — leaving it unset changes nothing.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| `CLARITY_API_TOKEN`                       | No           | Microsoft Clarity Data Export token. Lets the brain read session-recording and heatmap metrics (dead clicks, rage clicks, JS errors) through `/api/mcp` → `query_external_service`. Clarity is already live on the site via `public/clarity-init.js`; this only adds READ access for the brain. Its export API covers the last 1–3 days, so it is current state rather than history. **Ten requests per project per day**, and that cannot be raised by paying — Clarity has no paid tier. The `brain-clarity` cron (05:40 UTC) spends one and writes a per-page frustration summary into the corpus, so search the corpus (source `clarity`) before calling the API.                                                                                                                                                                                                                                                                                                                 |
...
| `SUPABASE_TEST_URL`                       | No           | Optional override for the read-only `rls-boundary` integration test, which otherwise runs against `SUPABASE_URL`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `SUPABASE_TEST_ANON_KEY`                  | No           | Optional override, paired with `SUPABASE_TEST_URL`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| `BRAIN_LLM_BASE_URL`                      | No           | OpenAI-compatible base URL for the brain (default `https://generativelanguage.googleapis.com/v1beta/openai/`, i.e. Gemini). The whole point of writing against that shape: Gemini, Groq (`https://api.groq.com/openai/v1`), OpenAI or a local Ollama swap with this one value and no code change. `https://api.anthropic.com/v1` switches to Claude's native Messages API (model default `claude-sonnet-5`); Anthropic's own OpenAI-compatible layer is documented as not production-ready, so it is not used.                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| `BRAIN_LLM_CLI`                           | No           | `claude` sends every brain model call through the `claude -p` binary instead of an HTTP API, and wins over `BRAIN_LLM_KEY`. This is how the brief and the decision miner use the **Team subscription** instead of a billed API key: `.github/workflows/brain-daily.yml` sets it, with `CLAUDE_CODE_OAUTH_TOKEN` (from `claude setup-token`) as the credential. Anthropic's terms allow that token only in the unmodified binary, never in our own API calls. Model defaults to the `sonnet` alias (`BRAIN_LLM_MODEL` overrides). Vercel has no `claude` binary, so never set it there. On a laptop it uses whoever is logged in to Claude Code.                                                                                                                                                                                                                                                                                                                                       |
| `BRAIN_RESEARCH_MODEL`                    | No           | The Night Shift's model (`brain-daily.yml`, 00:30 UTC, questions queued with `queue_research`). Default the `sonnet` alias; `opus` researches deeper at a higher cost to the seat's allowance. GitHub Actions only.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `BRAIN_MCP_URL`                           | No           | Where the Night Shift's researcher and the Claude Code session hook reach the brain. Default `https://www.loveiq.org/api/mcp` (the www host: the apex redirect drops the `Authorization` header). They authenticate with `LOVEIQ_MCP_TOKEN`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `SLACK_BRAIN_BOT_TOKEN`                   | For brain    | Bot token for the brain's **own** Slack app — deliberately separate from `SLACK_BOT_TOKEN`, which drives the live journey messages; adding event subscriptions and `im:*` scopes to that app would force a reinstall and risk breaking a working integration. Scopes: `app_mentions:read`, `chat:write`, `im:history`, `im:read`, `im:write`, plus `channels:history` + `channels:read` for public-channel ingest. **For private channels and group DMs the app also needs `groups:history` + `groups:read` and `mpim:history` + `mpim:read`, then a reinstall.** Scopes alone grant nothing: Slack only returns conversations the bot has been INVITED to, so each private channel or group DM has to add it. Until those scopes exist the ingester detects the `missing_scope` answer and falls back to public channels — `conversations.list` fails the whole call on one unsupported type, so without that fallback asking for private channels would stop Slack ingest entirely. |
...
| `LOVEIQ_MCP_TOKEN`                        | For MCP      | The SHARED bearer for `/api/mcp`, for the jobs that run unattended (the Night Shift, the weekly health report, the test batteries, the session hook) and the committed `.mcp.json`. People sign in to Jarvis as themselves since 2026-09-26 (Supabase OAuth, see "Connecting Claude to it" in `docs/runbooks/COMPANY_BRAIN.md`), so their calls and writes carry their own name. Treat this token as read access to everything indexed. Unset, only personal sign-ins work. **Point clients at `https://www.loveiq.org/api/mcp`** — the apex-to-www redirect drops the `Authorization` header and presents as a confusing 401.                                                                                                                                                                                                                                                                                                                                                        |
...
- `brain-daily.yml` — The company brain's daily brief and decision miner, run through `claude -p` on the Team subscription (`CLAUDE_CODE_OAUTH_TOKEN`), not on Vercel
...
1. `SLACK_BRAIN_TEAM_ID` is the only thing stopping another Slack workspace from
   reading all of it, and `LOVEIQ_MCP_TOKEN` (the unattended jobs' shared token) or a
   member's own sign-in opens all of it too, so a leak of any of them is a leak of the
   whole corpus. The check reads the **speaker's**
