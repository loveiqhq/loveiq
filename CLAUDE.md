...
| `STRIPE_LIVE_MODE`                         | No           | T-01 livemode guard. Set to `true` on prod Vercel project pointing at Stripe live mode; leave unset/`false` in dev/sandbox. Webhook events whose `event.livemode` does not match this env are refused (with Slack ops alert) so test events never fulfill against prod data. |
