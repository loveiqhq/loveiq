...
10. **Nurture Email Sequence:** `/api/cron/nurture-sequence` (hourly) reads `personal_report.created_date_time` and fans out to **two** stages — `72h_no_unlock` and `78h_no_unlock` (`type Stage` in the route is the source of truth). `72h_no_unlock` is the **only** nurture email in practice: it mints a per-user Stripe Promotion Code from `STRIPE_COUPON_50` (24h expiry, customer-restricted) → Resend send → idempotency write to `report_price_quote.metadata.nurtureEmailsSent[]` + `nurturePromoCodes[stage]`. `78h_no_unlock` is the call invite and is **paused by default** (`NURTURE_78H_CALL_ENABLED`), so an unconverted reader receives exactly one follow-up. Pricing 2.0 retired the earlier ladder — the 6h "report ready"/"unlock" reminders and the 30h/54h escalating discounts are gone; do not reintroduce them from an older copy of this file. Email CTA links carry `?promo=<code>&offer=1&pricingSessionId=…&utm_campaign=<stage>`. The `/report/[token]` page stashes `?promo=` in sessionStorage; checkout-session POST forwards it, server validates ownership via `resolveNurturePromo()`, pre-applies as `discounts:[]` on the Stripe session, and stamps `metadata.promoCode`/`metadata.promoStage` for attribution.
