...
| `BRAIN_LLM_KEY`                            | For brain    | API key for the company-brain answer model. Default provider is Groq's free tier (no card). Unlike Google's free tier, Groq does not train on the prompts we send — which matters because those prompts carry our docs, commits and Jira. Unset ⇒ the brain replies that it has no model configured, and nothing else breaks.                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `BRAIN_LLM_BASE_URL`                       | No           | OpenAI-compatible base URL for the brain (default `https://api.groq.com/openai/v1`). The whole point of writing against that shape: Groq, Gemini, OpenAI or a local Ollama swap with this one value and no code change.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `BRAIN_LLM_MODEL`                          | No           | Brain model id (default `groq/compound`: 30 req/min, 250/day, 70K tokens/min free). `openai/gpt-oss-120b` is a plainer non-agentic alternative but its free tier allows only 8K tokens/min — roughly one question a minute once sources are in the prompt. Prompts are sized to fit either.                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| `JIRA_API_TOKEN`                           | For brain    | Atlassian API token ([create one](https://id.atlassian.com/manage-profile/security/api-tokens)). `/api/cron/brain-ingest` returns `{skipped:true}` when this, `JIRA_EMAIL` or `JIRA_BASE_URL` is unset.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| `GOOGLE_OAUTH_REFRESH_TOKEN`               | For brain    | Long-lived refresh token, exchanged for an access token per run. Scopes are fixed at login — a missing one surfaces as a 403 "insufficient authentication scopes" error, which the ingester rewrites into the command that fixes it.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
