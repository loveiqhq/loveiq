...
interface BulkActionBarProps {
  ...
  /** True when EVERY selected row's `is_likely_test === true`. */
  allSelectedAreTests: boolean;
  onClear: () => void;
  onComplete: () => void;
}
...
export default function BulkActionBar({
  ...
  async function handleDelete() {
    ...
    try {
      ...
      if (!res.ok) {
        const body = await res.json().catch(() => null);
        setError((body as { error?: string } | null)?.error || "Delete failed.");
        return;
      }
  ...
  const deleteDisabled = loading || !allSelectedAreTests;
  const deleteTooltip = !allSelectedAreTests
    ? "Selection includes non-test submissions. Filter to “Test only” to enable delete."
    : `Delete ${selectedIds.size} test submission${selectedIds.size === 1 ? "" : "s"}.`;
      ...
      <div className="sticky bottom-4 z-30 mx-auto w-fit rounded-xl border border-white/10 bg-surface px-5 py-3 shadow-lg shadow-black/40 backdrop-blur-sm">
        <div className="flex items-center gap-4">
          ...
          <button
            ...
            title={deleteTooltip}
            className="rounded-lg border border-red-500/30 px-3 py-1.5 text-xs font-medium text-red-300 transition hover:bg-red-500/10 disabled:opacity-30 disabled:cursor-not-allowed"
          >
            Delete
          </button>
          {selectedIds.size === 2 && (
        ...
        {deleteSummary && <p className="mt-2 text-xs text-text-muted">{deleteSummary}</p>}
      </div>

      <ConfirmDialog
        open={showDeleteConfirm}
        title={`Delete ${selectedIds.size} test submission${selectedIds.size === 1 ? "" : "s"}?`}
        message="This permanently removes the submission, its answers, scoring, and analytics events. Submissions with a paid report are preserved automatically. This cannot be undone."
        confirmLabel="Delete"
        requireTyped="DELETE"
