-- Down for 20260906002812_brain_search_recency_and_wider_stage1.
--
-- Restores the previous body verbatim: no recency term, stage-1 cut back to 150.
-- The signature is identical in both directions, so this is a replacement rather
-- than an overload and needs no DROP.
--
-- Reverting costs the two things the forward migration bought: the ranker stops
-- being able to tell a decision from two days ago from a commit from March, and
-- `limit` under-delivers again -- measured, asking for 15 sources returned 9.
