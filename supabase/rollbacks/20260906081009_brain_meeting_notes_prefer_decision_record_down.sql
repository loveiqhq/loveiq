-- Down for 20260906081009_brain_meeting_notes_prefer_decision_record.
--
-- Removes the dedup preference and the `section` tags. Reverting restores the
-- measured behaviour it fixed: on roughly a third of meetings the decision record
-- is discarded at random and a transcript fragment is returned in its place.
--
-- The section tags are cheap to rebuild -- re-run the backfill in the forward
-- migration, or let the drive ingester rewrite the notes -- so this down is safe
-- in a way the corpus deletion migration is not.

UPDATE public.brain_chunk
   SET meta = meta - 'section'
 WHERE source = 'drive' AND meta ? 'section';
