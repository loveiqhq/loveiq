-- Caller-passed filters: source, date range, and metadata containment.
--
-- WHY. The metadata to answer several failing questions was already in `meta` and
-- simply unreachable. 1,085 Notion board rows carry `status`, `assignee`, `priority`
-- and `due`; slack carries `channel` and `day`; commit carries `author` and `date`;
-- gmail carries `mailbox`. Measured before this: "which notion tasks are work in
-- progress" put the authoritative row at rank 4 while commits took 3 of the top 5
-- slots -- and 49 rows literally carry `status: WIP`. Guessing keywords is not the
-- same as being able to narrow.
--
-- THE PREDICATE IS APPLIED TWICE, ON PURPOSE, and this is the easy part to get wrong.
-- The `vec` arm is a top-120 over the WHOLE corpus. Filtering only afterwards means a
-- narrow request like sources=['notion'] keeps whichever of those 120 global
-- neighbours happen to be notion -- for a 1,418-row source out of 25,000, usually
-- none. The semantic arm would silently stop working exactly when the caller was most
-- specific. So the filter goes INSIDE `vec`, and again in `cheap` for the lexical
-- arms, which are not top-N limited and so are safe to filter late.
--
-- DATE FILTERS EXCLUDE UNDATED ROWS, deliberately. `doc` chunks -- the repo's own
-- markdown, 487 of them -- carry no `period_end`, so any `since`/`until` drops all of
-- them. The alternative (`period_end IS NULL OR ...`) makes "what happened last week"
-- return timeless policy docs, which is worse. The tool description says so out loud.
--
-- NO INDEX ON `meta`, measured rather than assumed: `meta @> '{"status":"WIP"}'` is a
-- 28ms seq scan, all shared hits, and inside this function it runs against the
-- `hits`-joined set which is far smaller. A GIN index would cost 10-20MB AND be
-- rewritten by every `touchChunks` PATCH -- on a project whose Disk IO budget was
-- exhausted on 2026-08-31 by exactly that write pattern.
--
-- THE DROP AT THE BOTTOM IS NOT OPTIONAL. Adding parameters WITH DEFAULTS creates an
-- OVERLOAD, not a replacement, and Postgres then cannot resolve the deployed app's
-- four-argument call at all. That took production search down on 2026-08-30
-- (20260830100000_brain_search_semantic.sql:110-123). Ordered AFTER the create, so a
-- failed create leaves the old function standing.
--
-- Deploy safety: PostgREST resolves by the JSON keys present, so an old app version
-- sending four keys resolves to the new function and takes the defaults.
--
-- Verified after applying: one signature; the four-argument call still returns 12
-- rows; sources+meta returns 30 rows of which 30 are notion AND 30 are status=WIP;
-- exclude_sources removes every excluded row; since= returns 0 undated rows.
...
CREATE OR REPLACE FUNCTION public.brain_search(
  ...
  exclude_sources text[] DEFAULT NULL,
...
-- See the header. Adding defaulted parameters OVERLOADS rather than replaces, and the
-- deployed four-argument call then resolves to neither.
DROP FUNCTION IF EXISTS public.brain_search(text, integer, integer, halfvec);
