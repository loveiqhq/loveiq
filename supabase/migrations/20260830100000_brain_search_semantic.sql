-- Semantic recall in the ranker.
--
-- Lexical search can only match words that are present. Measured on the real
-- corpus, before and after:
--
--   "why do people give up before paying"
--     lexical  -> an article titled "Why Hurt People Hurt People"
--     semantic -> the "remove pay-first" and "paywall countdown" commits
--   "what worries do customers raise about privacy"
--     lexical  -> "Add more assets to reach more customers"
--     semantic -> DPIA.md, the privacy commits, the consent checklist
--
-- Lexical matched the WORDS; semantic found the DOCUMENTS.
--
-- The query vector is computed by the CALLER and passed in, because Postgres cannot
-- run the model. `query_embedding` is optional and everything degrades to the
-- previous lexical behaviour when it is null — so an embedding outage makes search
-- worse, never broken.
--
-- Cost measured warm: 113ms against 96ms lexical-only.
...
-- Adding a fourth parameter WITH A DEFAULT does not replace the three-argument
-- function from 20260829120000 -- it creates a second, overloaded one. Postgres
-- then cannot resolve a three-argument call at all:
--
--   ERROR 42725: function brain_search(unknown, integer, integer) is not unique
--
-- The deployed app calls with three arguments until this ships, so leaving both in
-- place breaks every search. This drop is what makes the old call resolve to the
-- new default-bearing function, letting old and new app versions run side by side
-- through the deploy. It is ordered AFTER the create on purpose: if the create
-- above fails, the old function is still standing and nothing is down.
-- (Learned the hard way -- this exact overload took production search down on
-- 2026-08-30 for the ~2 minutes it took to notice.)
...
-- lists and degrades as the corpus outgrows what it was trained on. HNSW has no
-- training step, so it stays correct as sources are added. Cosine ops because the
-- vectors are L2-normalised (verified: norm 1.0000).
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_brain_chunk_embedding
  ON public.brain_chunk USING hnsw (embedding halfvec_cosine_ops);

DROP FUNCTION IF EXISTS public.brain_search_probe(text, integer, integer);
DROP FUNCTION IF EXISTS public.brain_search_probe(text, integer, integer, halfvec);
