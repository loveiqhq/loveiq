-- Migration: Create get_predictive_insights RPC for admin predictive analytics
-- Run this in your Supabase SQL Editor
--
-- Computes 7 types of predictive insights from existing tables:
--   1. Volume Projection (survey_submission)
--   2. Abandonment Predictors (survey_behavior_event)
--   3. UTM Conversion Rates (survey_submission + survey_behavior_event)
--   4. Archetype Trend (scoring_result + survey_submission)
--   5. Friction Zone Detection (survey_behavior_event)
--   6. Completion Time Prediction (survey_behavior_event + survey_submission)
--   7. Revenue Forecast (survey_submission + payment)
...
BEGIN
  ...
  -- =========================================================================
  -- 1. Volume Projection
  -- Compare daily submission count in last p_days vs prior p_days period
  -- =========================================================================
  ...
  -- For each question, compute: of sessions where time_spent_ms was in the
  -- bottom 25th percentile, what % had direction='abandon' later?
  -- =========================================================================
  BEGIN
  ...
  EXCEPTION WHEN OTHERS THEN
    -- Graceful degradation: skip this insight on error
    NULL;
  END;

  -- =========================================================================
  -- 3. UTM Conversion Rates
  -- For each utm_source, compute conversion rate. Surface sources >1.5x overall avg.
  -- =========================================================================
  BEGIN
  ...
  -- =========================================================================
  -- 4. Archetype Trend
  -- Compare archetype distribution in last p_days/2 vs prior p_days/2.
  -- Surface archetypes with >15% relative change.
  -- =========================================================================
  BEGIN
    WITH recent_half AS (
  ...
  -- Find consecutive question ranges where abandon rate >2x survey average.
  -- =========================================================================
  BEGIN
  ...
  -- For users who averaged >2min per question in questions 0-9,
  -- what was their median total duration vs overall median?
  -- =========================================================================
  BEGIN
    WITH early_avg AS (
      SELECT
      ...
      WHERE created_at >= v_period_start
        ...
        AND time_spent_ms > 0
      GROUP BY session_id
      HAVING AVG(time_spent_ms) > 120000  -- >2 minutes per question
    ),
    slow_durations AS (
  ...
  -- =========================================================================
  -- 7. Revenue Forecast
  -- Projected 30-day revenue based on submission rate, conversion, avg payment.
  -- =========================================================================
  BEGIN
    -- Daily submission rate in current period
    SELECT COUNT(*)::NUMERIC / GREATEST(p_days, 1)
    INTO v_daily_submissions
    FROM survey_submission
    WHERE created_date_time >= v_period_start;

    -- Payment conversion rate and average amount
    SELECT
      CASE WHEN COUNT(*) > 0
...
-- Grant execute to service_role only (no public access)
