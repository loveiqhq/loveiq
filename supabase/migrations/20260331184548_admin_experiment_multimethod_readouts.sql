ALTER TABLE public.admin_experiment
  ADD COLUMN IF NOT EXISTS control_metric_value numeric,
  ADD COLUMN IF NOT EXISTS variant_metric_value numeric,
  ADD COLUMN IF NOT EXISTS control_stddev_value numeric,
  ADD COLUMN IF NOT EXISTS variant_stddev_value numeric;

ALTER TABLE public.admin_experiment
  DROP CONSTRAINT IF EXISTS admin_experiment_readout_method_check;

ALTER TABLE public.admin_experiment
  ADD CONSTRAINT admin_experiment_readout_method_check
  CHECK (
    readout_method IS NULL OR
    readout_method IN ('conversion-rate', 'count-delta', 'average-value')
  );
...
ALTER TABLE public.admin_experiment
  ...
  CHECK (
    ...
    (variant_sample_size IS NULL OR variant_success_count IS NULL OR variant_success_count <= variant_sample_size) AND
    (control_metric_value IS NULL OR control_metric_value >= 0) AND
    (variant_metric_value IS NULL OR variant_metric_value >= 0) AND
    (control_stddev_value IS NULL OR control_stddev_value >= 0) AND
    (variant_stddev_value IS NULL OR variant_stddev_value >= 0)
  );

DROP FUNCTION IF EXISTS public.admin_upsert_experiment(
  text,
  bigint,
  text,
  text,
...
CREATE OR REPLACE FUNCTION public.admin_upsert_experiment(
  ...
  p_control_metric_value numeric DEFAULT NULL,
  p_variant_metric_value numeric DEFAULT NULL,
  p_control_stddev_value numeric DEFAULT NULL,
  p_variant_stddev_value numeric DEFAULT NULL,
...
BEGIN
  ...
  IF p_readout_method IS NOT NULL
    AND p_readout_method NOT IN ('conversion-rate', 'count-delta', 'average-value')
  THEN
    RAISE EXCEPTION 'unsupported readout method';
  END IF;

  IF p_readout_method = 'average-value' THEN
    IF (p_control_sample_size IS NOT NULL AND p_control_sample_size <= 1)
      OR (p_variant_sample_size IS NOT NULL AND p_variant_sample_size <= 1)
    THEN
      RAISE EXCEPTION 'average-value readouts require at least 2 observations per group';
    END IF;
  END IF;
  ...
  ELSE
    ...
    SET owner_email = p_owner_email,
        ...
        control_metric_value = p_control_metric_value,
        variant_metric_value = p_variant_metric_value,
        control_stddev_value = p_control_stddev_value,
        variant_stddev_value = p_variant_stddev_value,
