...
/**
 * Early-access invitation email — sent to waitlist members at launch via a
 * Resend broadcast (NOT triggered by signup). Figma node 4454-4096.
 */
export interface WaitlistEarlyAccessEmailParams {
  firstName?: string | null;
  /** Absolute site URL — used to build the survey CTA. */
  siteUrl: string;
}

export function waitlistEarlyAccessEmail({ firstName, siteUrl }: WaitlistEarlyAccessEmailParams) {
  const safeFirstName = firstName?.trim() ? escapeHtml(firstName.trim()) : "there";
  const greetingText = firstName?.trim() || "there";

  const subject = "You are chosen \u2014 early access to your LoveIQ insights";
  const previewText =
    "LoveIQ is now live and you\u2019re among the first early users invited from our waitlist.";
  ...
  <tr>
    <td style="padding:24px 32px 8px;">
      <h1 style="margin:0; font-family:${EMAIL_FONT}; font-size:26px; font-weight:600; line-height:1.35; color:#000000; letter-spacing:-0.3px;">
        You are chosen for early access!
      </h1>
    </td>
  </tr>
  <tr>
    <td style="padding:16px 32px 0;">
      ...
      <p style="margin:0 0 16px 0; font-family:${EMAIL_FONT}; font-size:17px; line-height:1.55; color:#000000;">
        After years of research and months of building, LoveIQ is now live and you&rsquo;re among the first early users invited from our waitlist.
      </p>
  ...
  const text = [
    "You are chosen — early access to your LoveIQ insights",
    "",
    `Hi ${greetingText},`,
    "",
    "After years of research and months of building, LoveIQ is now live and you're among the first early users invited from our waitlist.",
    "",
    "Early feedback has been strong:",
