...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!response.ok) {
      // warn-not-error: best-effort feedback (user can re-vote on transient
      // failure). Breaker fires `circuit_open` separately if sustained.
      let respBody = "";
      try {
      ...
      logger.warn(
        ...
        "Supabase report feedback upsert non-2xx"
      );
    ...
    if (err instanceof CircuitOpenError) {
      ...
      logger.warn({ err }, "Supabase error on report feedback");
    }
