...
const makeReq = (body: unknown) =>
  new Request("http://localhost/api/admin/alerts/1", {
    method: "PATCH",
    headers: { "content-type": "application/json" },
    body: JSON.stringify(body),
  });

describe("admin alerts/[id] PATCH", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("returns 400 when comparator is not in enum", async () => {
    const res = await PATCH(makeReq({ comparator: "approx" }), makeParams("1"));
    expect(res.status).toBe(400);
  });

  it("returns 400 when severity is invalid", async () => {
    const res = await PATCH(makeReq({ severity: "critical" }), makeParams("1"));
    expect(res.status).toBe(400);
  });
