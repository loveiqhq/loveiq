...
vi.mock("@features/admin/server/path-intelligence", async () => {
  const actual = await vi.importActual<typeof import("@features/admin/server/path-intelligence")>(
    "@features/admin/server/path-intelligence"
  );
  return {
    ...actual,
...
describe("GET /api/admin/path-intelligence", () => {
  ...
  it("returns 429 when rate-limited", async () => {
    ...
    expect(mockBuildAdminPathIntelligenceSnapshot).not.toHaveBeenCalled();
  });
});
