...
import { GET } from "@/app/api/admin/pulse/at-risk/route";

describe("admin pulse/at-risk route", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("returns 401 without admin session", async () => {
    ...
    const res = await GET(new Request("http://localhost/api/admin/pulse/at-risk"));
  ...
  it("returns 200 with empty list when no at-risk sessions exist", async () => {
    mockSupabaseFetch.mockResolvedValue({ ok: true, json: async () => [] });
    const res = await GET(new Request("http://localhost/api/admin/pulse/at-risk"));
    expect(res.status).toBe(200);
  });
