...
import { GET, POST } from "@/app/api/admin/tag-rules/route";

function makeRequest(body?: unknown, method: "GET" | "POST" = "GET"): Request {
  return new Request("http://localhost/api/admin/tag-rules", {
    method,
...
describe("admin tag-rules route", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("POST create returns 400 when field is not in enum", async () => {
    const res = await POST(makeRequest({ ...validCreate, field: "random" }, "POST"));
    expect(res.status).toBe(400);
  });

  it("POST create returns 400 when operator is not in enum", async () => {
    const res = await POST(makeRequest({ ...validCreate, operator: "xyz" }, "POST"));
    expect(res.status).toBe(400);
  });

  it("POST toggle returns 400 when ruleId missing", async () => {
    const res = await POST(makeRequest({ action: "toggle" }, "POST"));
    expect(res.status).toBe(400);
  });

  it("POST delete returns 403 for editor (delete requires admin)", async () => {
    mockVerifyAdminSession.mockResolvedValue({ email: "e@test.com", role: "editor" });
    const res = await POST(makeRequest({ action: "delete", ruleId: 7 }, "POST"));
    expect(res.status).toBe(403);
  });
});
