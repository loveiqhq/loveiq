...
const makeReq = (body: unknown) =>
  new Request("http://localhost/api/admin/reviews/1", {
    method: "PATCH",
    headers: { "content-type": "application/json" },
    body: JSON.stringify(body),
  });

describe("admin reviews/[id] PATCH", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("returns 401 without admin session", async () => {
    ...
    const res = await PATCH(makeReq({ status: "approved" }), makeParams("1"));
  ...
  it("returns 400 when id is non-numeric", async () => {
    const res = await PATCH(makeReq({ status: "approved" }), makeParams("abc"));
    expect(res.status).toBe(400);
  });

  it("returns 400 when status is not in enum", async () => {
    const res = await PATCH(makeReq({ status: "URGENT" }), makeParams("1"));
    expect(res.status).toBe(400);
  });
