...
vi.mock("@features/admin/server/metric-registry", () => ({
  buildMetricRegistrySnapshot: vi.fn().mockResolvedValue({ entries: [] }),
}));
...
import { GET, POST } from "@/app/api/admin/metric-registry/route";

function makeRequest(body?: unknown, method: "GET" | "POST" = "GET"): Request {
  return new Request("http://localhost/api/admin/metric-registry", {
    method,
...
describe("admin metric-registry route", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("POST returns 403 for viewer role (writes require editor)", async () => {
    mockVerifyAdminSession.mockResolvedValue({ email: "v@test.com", role: "viewer" });
    const res = await POST(makeRequest({ action: "create", metric_key: "x", label: "X" }, "POST"));
  ...
  it("POST returns 400 when action is missing (discriminator)", async () => {
    const res = await POST(makeRequest({ metric_key: "x", label: "X" }, "POST"));
    expect(res.status).toBe(400);
  });

  it("POST returns 400 when unit is not in the enum", async () => {
    const res = await POST(
      makeRequest({ action: "create", metric_key: "x", label: "X", unit: "joules" }, "POST")
    );
    expect(res.status).toBe(400);
  });

  it("POST returns 400 when review action lacks registryId", async () => {
    const res = await POST(makeRequest({ action: "review" }, "POST"));
    expect(res.status).toBe(400);
  });
});
