...
import { GET, POST } from "@/app/api/admin/alerts/route";

function makeRequest(body?: unknown, method: "GET" | "POST" = "GET"): Request {
  return new Request("https://example.test/api/admin/alerts", {
    method,
...
describe("admin alerts route", () => {
  ...
  it("POST returns 400 when label too short", async () => {
    mockVerifyAdminSession.mockResolvedValue({ email: "a@test.com", role: "admin" });
    const res = await POST(
      makeRequest(
        {
  ...
  it("POST returns 400 when comparator not in enum", async () => {
    mockVerifyAdminSession.mockResolvedValue({ email: "a@test.com", role: "admin" });
    const res = await POST(
      makeRequest(
        {
