...
import { GET, POST } from "@/app/api/admin/views/route";

function makeRequest(body?: unknown, method: "GET" | "POST" = "GET"): Request {
  return new Request("http://localhost/api/admin/views", {
    method,
...
describe("admin views route", () => {
  ...
  it("POST returns 403 for viewer role (views require editor)", async () => {
  ...
  it("POST returns 400 when filters object is missing fields", async () => {
    const res = await POST(makeRequest({ name: "x", filters: { status: "" } }, "POST"));
    expect(res.status).toBe(400);
  });
});
