...
const TABS = ["Lineage Graph", "Partial Analytics"] as const;
type Tab = (typeof TABS)[number];

export default function JourneyDashboard() {
  const [days, setDays] = useState(0);
  const [activeTab, setActiveTab] = useState<Tab>("Lineage Graph");
      ...
      {activeTab === "Lineage Graph" && (
        <div className="space-y-6">
          <div className="rounded-xl border border-white/10 bg-surface p-5">
            <h3 className="mb-4 text-sm font-medium text-text-primary">Response Lineage Graph</h3>
            <SankeyDiagram nodes={data.nodes} links={data.links} />
          </div>

          <div className="rounded-xl border border-white/10 bg-surface p-5">
            <h3 className="mb-4 text-sm font-medium text-text-primary">Lineage By Entry Path</h3>
              ...
              <table className="w-full text-left text-sm">
                ...
                <tbody>
                  {data.cohorts.map((cohort) => (
                    <tr key={cohort.path} className="border-b border-white/5 hover:bg-white/5">
                      <td className="px-4 py-3 text-text-primary">{cohort.path}</td>
                      <td className="px-4 py-3 text-text-muted">{cohort.total}</td>
                      <td className="px-4 py-3 text-text-muted">{cohort.completionRate}%</td>
      ...
      {activeTab === "Partial Analytics" && (
        <div className="space-y-6">
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <StatCard label="Avg Checkpoint" value={`Q${data.partialAnalytics.avgCheckpoint}`} />
            <StatCard
              label="Top Placement"
          ...
          <div className="grid gap-6 xl:grid-cols-2">
            <div className="rounded-xl border border-white/10 bg-surface p-5">
              <h3 className="mb-4 text-sm font-medium text-text-primary">Checkpoint Recovery</h3>
            ...
            <div className="rounded-xl border border-white/10 bg-surface p-5">
              ...
              <div className="space-y-3">
                {data.partialAnalytics.placementSegments.map((segment) => (
                  ...
                  >
                    <div className="flex items-center justify-between gap-3">
                      <p className="text-sm font-semibold text-text-primary">{segment.placement}</p>
                      <p className="text-sm text-text-muted">{segment.recoveryRate}% recovery</p>
                    </div>
                    <p className="mt-1 text-xs text-text-muted">
