...
- `server/llm.ts` — one `fetch` to an OpenAI-compatible endpoint (Groq by default)
...
- **Ingestion.** Docs and commits come from `scripts/brain-ingest-repo.mjs` (run by
  ...
  the git history actually on disk). Jira comes from
  `app/api/cron/brain-ingest/route.ts`.
- **Metrics.** PostHog, Stripe, Vercel and GA4 numbers are not indexed. An indexed
  metric is a stale metric, and a brain that confidently quotes last week's
  conversion rate is worse than one that says it does not know. Those belong in a
  live tool-call layer.
