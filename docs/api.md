...
| `/api/cron/brain-ingest`              | `GET`       | Scheduled job (daily 04:47 UTC): ingests Jira issues from `GROW`/`SCRUM`/`SHOWUP` into `brain_chunk` for the company brain, via `POST /rest/api/3/search/jql` with `nextPageToken` paging and a 45s time budget. Flattens Atlassian Document Format descriptions to text. Sweeps deleted issues only when a run walked every page. Auth: `CRON_SECRET`; returns `{skipped:true}` when Jira credentials are unset. The repo half of the corpus (docs + commits) is ingested by the `brain-ingest` GitHub Action on push instead.                                                |
