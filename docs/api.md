...
| `/api/cron/nurture-sequence`          | `GET`       | Scheduled job: post-survey nurture emails (4 stages: 6h_no_view, 6h_no_unlock, 30h, 54h). Mints per-user Stripe Promotion Codes at the discounted stages. Authenticated via `CRON_SECRET`.                                                                                                                   |
...
| `/api/cron/abandoned-checkout-alert`  | `GET`       | Scheduled job (every 30 min): detects abandoned checkouts (begin_checkout > 30m, no succeeded payment) and pings Slack ops. Deduped via `slack_alert_sent`. Auth: `CRON_SECRET`.                                                                                                                             |
...
| `/api/stripe/prepaid-checkout`        | `POST`      | White "pay-first" cohort: creates a Stripe Checkout session at the fixed full-report price BEFORE a submission exists, mints the prepaid entitlement + httpOnly bearer token. White cohort only (403 otherwise).                                                                                             |
| `/api/stripe/prepaid-status`          | `GET`       | Polls whether the white pay-first prepaid entitlement has been paid (reconciles against Stripe if the webhook is slow); used by the survey pay-gate interstitial.                                                                                                                                            |
