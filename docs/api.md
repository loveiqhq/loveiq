...
| `/api/slack/events`                   | `POST`      | Company-brain Slack front door. Answers an `app_mention` or a DM in-thread from `brain_chunk` (docs, commits, Jira, funnel numbers, GA4, Search Console) with cited sources. Verified via the Slack `v0` request signature (no CSRF); 503 until `SLACK_BRAIN_SIGNING_SECRET` is set. Acknowledges within Slack's 3s deadline and answers afterwards; deduped on Slack's `event_id` via a UNIQUE claim in `brain_query`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
