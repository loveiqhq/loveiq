...
| `/api/cron/brain-drive`               | `GET`       | Scheduled job (every 15 min, at 7/22/37/52 past): ingests Google Drive documents shared with `ga4-reader@loveiq-brain.iam.gserviceaccount.com` into `brain_chunk` — in practice the "Notes by Gemini" written after each call, so a meeting is searchable within minutes instead of the next morning. Separate from `brain-ingest` on purpose: a Drive run with nothing new is ONE list call, whereas that job crawls 35 Notion databases and both Google properties and costs ~30s. `brain-ingest` also ingests Drive nightly, so a failure here degrades to daily rather than to nothing. `drive-nothing-shared` and `google-not-configured` are silent by design; anything else alerts ops once per day. Auth: `CRON_SECRET`. Returns 200 even on failure so Vercel does not retry a job that will fail again in 15 minutes. |
