...
**Thirty-five tools, in three groups.** Twenty-six read, nine write. The write ones act
...
| `user_totals`            | Anonymous totals about our users: how many finished the survey, how many paid and what they paid, by up to two of gender, age band, orientation, relationship, country, archetype and month, narrowed by the same keys. Never a person: a group under 5 is hidden, and when only one would be, the next-smallest goes with it so subtraction cannot reveal it (decision of 26 Sep 2026). Staff left out; paid is a real sale above EUR 0                                        |
| `check_answer`           | Before an answer is sent: every number, date and quoted phrase in the draft is looked up in the documents it cites, and anything not there is listed with the nearest figure the source holds. A sentence that names an id is checked against that document alone. Deterministic: it cannot judge wording, and says which sentences it could not check                                                                                                                          |
