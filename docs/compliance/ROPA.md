...
**What actually leaves the building:** repo documentation, commit messages, the
Notion workspace (the team board and its pages) and aggregate business numbers.
Jira is listed in the code but has never been ingested — 0 chunks — and Notion
replaced it on 2026-08-28.

There IS personal data, and it is the team's own: commit messages carry
contributor names and git email addresses, and board tasks carry the assignee's
name. No survey answers, no archetype output, no customer email addresses, no
payment data.
